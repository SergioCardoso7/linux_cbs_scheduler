...
keep_resched:
    ...
    if (cbs_policy(prev->policy) || cbs_policy(next->policy))
        should_trace = 1;
    #endif
...
#ifdef CONFIG_MOKER_SCHED_LIFO_POLICY
	if (lf_policy(policy))
		return &lf_sched_class;
#endif

#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
	if (cbs_policy(policy))
		return &cbs_sched_class;
#endif
	if (rt_prio(prio))
...
#ifdef CONFIG_MOKER_SCHED_LIFO_POLICY
	BUG_ON(!sched_class_above(&dl_sched_class, &lf_sched_class));
	BUG_ON(!sched_class_above(&lf_sched_class, &rt_sched_class));
#endif
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
	BUG_ON(!sched_class_above(&dl_sched_class, &cbs_sched_class));
	BUG_ON(!sched_class_above(&cbs_sched_class, &rt_sched_class));
#endif
